...
### Added
- **Native Contextual Search** (iOS 26+)
  - `AdaptiveCupertinoAppBar` now supports `AdaptiveCupertinoSearchOptions`.
  - Native `UISearchController` integration with Liquid Glass transition.
  - Context-aware search morphing from AppBar.
- **Refactored TabBar** (Cleaned up search logic)
  - Removed modal search presentation from `AdaptiveCupertinoTabBar`.
  - Deprecated `searchOptions` in TabBar in favor of AppBar integration.
